...
Uso:
    ...
    python stream/capture_frames.py \
        --source mjpeg \
...
def fetch_snapshot(url: str):
    """
    Busca um único frame JPEG via HTTP —
    sem streaming, sem buffer acumulado.
    """
...
def parse_args():
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Captura manual: pressione ENTER a cada frame, "
            "em vez de intervalo automático"
        )
    )
    ...
    p.add_argument(
        ...
        help=(
            "URL do endpoint /snapshot "
            "(usado apenas com --source mjpeg --manual)"
        )
    )
...
class RpicamCapture:
    """
    Captura frames da câmera CSI via rpicam-vid (MJPEG),
    com interface compatível com cv2.VideoCapture
    (.read(), .isOpened(), .release()).

    cv2.VideoCapture() não suporta libcamera
    (câmeras CSI no Raspberry Pi OS Bookworm/Trixie) —
    só funciona com V4L2 nativo (webcams USB).
    """
    ...
    def read(self):
        while True:
            start = self._buf.find(b"\xff\xd8")
                ...
                self._buf.find(b"\xff\xd9", start + 2)
                if start != -1
                else -1
            )
...
def open_capture(args):
    """
    Abre a câmera local (via rpicam-vid)
    ou o stream MJPEG configurado no pipeline.
    """
    ...
    if args.source == "mjpeg":
        ...
        # Câmera CSI/USB local:
        # cv2.VideoCapture não suporta libcamera,
        # por isso usamos rpicam-vid via subprocesso
    ...
    try:
        while saved < args.total:
            ...
            if args.manual:
                ...
                if args.source == "mjpeg":
                    # Requisição HTTP avulsa:
                    # sempre o frame atual, sem fila
                    ...
                    # Câmera direta:
                    # descarta o buffer acumulado durante a espera
