...
Uso:
python scripts/inspect_dataset.py --dataset dataset/exports/epi-v1/data.yaml
"""
...
def main():
    ...
    print(f"\n{'=' * 55}")
    print(
        f" Inspeção do Dataset: "
        f"{Path(args.dataset).parent.name}"
    )
    print(f"{'=' * 55}")
    ...
    for split in ["train", "valid", "test"]:
        ...
        if not labels_dir.exists():
            print(
                f" [{split}] AVISO: diretório não encontrado"
            )
            continue

        counts, missing = count_labels(
            labels_dir,
            nc
        )
