...
Avalia o mAP@0.5 de um pipeline de pré-processamento
no dataset epi-v1.
...
def evaluate_pipeline(
    ...
    """
    Avalia o mAP@0.5 do modelo com uma função
    de pré-processamento opcional.

    preprocess_fn:
        função que recebe um frame BGR NumPy e
        retorna um frame transformado.
    ...
    if preprocess_fn is None:
        ...
        images = (
            sorted(src_images_dir.glob("*.jpg"))
            + sorted(src_images_dir.glob("*.png"))
        )
        ...
        tmp_root = (
            Path("preprocessing/outputs/_tmp_eval")
            / safe_label
        )
